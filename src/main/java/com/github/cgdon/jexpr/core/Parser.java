...
import com.github.cgdon.jexpr.ext.Function;
...
public class Parser {
    ...
    switch (token.ttype) {
      ...
      case Token.TT_WORD: {
        ...
        for (Map.Entry<String, Function> entry : funcMap.entrySet()) {
          if (entry.getKey().equals(token.sval)) {
            ...
            if (func instanceof UnaryFunction) {
              ...
              Expr rand = parseExpr(0);
              expect(')');
              return ((UnaryFunction) func).setRand(rand);
  ...
  public void registerFunction(Class<? extends Function> funcClazz) {
    try {
      Function func = funcClazz.newInstance();
